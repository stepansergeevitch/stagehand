...
describe("wrapPsqlInRollback", () => {
    it("wraps a double-quoted -c payload in BEGIN … ROLLBACK", () => {
        const cmd = `PGPASSWORD="$PW" psql -h localhost -p 5433 -U northspyre -d ns_deal -v ON_ERROR_STOP=1 -c "UPDATE pro_forma_source SET x = 1 WHERE name = 'A&D loan'"`;
        expect(wrapPsqlInRollback(cmd)).toBe(`PGPASSWORD="$PW" psql -h localhost -p 5433 -U northspyre -d ns_deal -v ON_ERROR_STOP=1 -c "BEGIN; UPDATE pro_forma_source SET x = 1 WHERE name = 'A&D loan'; ROLLBACK;"`);
    });
