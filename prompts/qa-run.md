...
- **Seed first, yourself.** Perform the scenario's `Seed` steps exactly as written before step 1 — with SQL or the named repo script (one Bash call each; environment hints: {{seedHints}}), with the API (curl from Bash against the backend), or through the UI on the routes given for simple forms. Creating and editing the records the seed describes is allowed; anything else stays read-only. Seeding is never the reviewer's job: do not record `needs_human` because data is missing — if a seed step fails, record the scenario `blocked` with the exact seed step and error in `observation`, and continue with the next scenario. If a seed step would duplicate a record that already exists from an earlier pass (same name), reuse it. **Seed budget:** at most 15 tool calls per scenario's seed. Never edit spreadsheet-style tables (pro forma cells, lot mix rows) cell by cell in the browser to seed values — if a seed step asks for that, write the rows with SQL instead (find the table from the hints or the repo's models); if after 15 calls the seed is not in place, record the scenario `blocked: seed exceeded budget at step <n>` and move on.
