...
{{qaSetup}}
1. {{chromeSelect}} Then load the browser tools in ONE ToolSearch call: `select:mcp__claude-in-chrome__tabs_context_mcp,mcp__claude-in-chrome__tabs_create_mcp,mcp__claude-in-chrome__navigate,mcp__claude-in-chrome__resize_window,mcp__claude-in-chrome__read_page,mcp__claude-in-chrome__find,mcp__claude-in-chrome__form_input,mcp__claude-in-chrome__computer,mcp__claude-in-chrome__get_page_text,mcp__claude-in-chrome__read_network_requests`. If `tabs_context_mcp` errors with "extension is not connected", wait 5 seconds and call it once more before giving up (blocker: "browser extension not connected").
2. Call `tabs_context_mcp` first, then `tabs_create_mcp` — do not reuse existing tabs. Then call `resize_window` with width 1440 and height 900 on that tab, so every screenshot is a readable desktop viewport (a short or ultra-wide window produces unreadable strips); if the tool fails, continue anyway.
...
- **Seed first, yourself.** Perform the scenario's `Seed` steps exactly as written before step 1 — through the UI on the routes given, with the API (curl from Bash against the backend), with SQL, or with the named repo script. Creating and editing the records the seed describes is allowed; anything else stays read-only. Seeding is never the reviewer's job: do not record `needs_human` because data is missing — if a seed step fails, record the scenario `blocked` with the exact seed step and error in `observation`, and continue with the next scenario. If a seed step would duplicate a record that already exists from an earlier pass (same name), reuse it.
