...
2. **The rebase is logical, not textual.** Before replaying, diff what the base gained since this branch forked (`git log --stat HEAD..origin/{{baseBranch}}`, `git diff HEAD...origin/{{baseBranch}}` on the files this branch touches) and compare it with what this branch adds. Anything this branch introduced that the base now already provides — a model field, a constant or enum member, a migration adding the same column, a helper, a schema field, a test fixture — is redundant after the rebase: drop this branch's copy and use the base's, adapting names if the base chose different ones. Migrations in particular: after the rebase there must be exactly one chain with no second migration adding a column the base's migration already adds and no dangling leaf — remove the redundant migration file(s) from this branch's commits (or regenerate this branch's migration against the rebased models the way the repository does it) rather than adding a merge migration on top. A commit whose whole content became redundant is dropped from the replay (`git rebase` with `--onto`/skip, or an empty-commit skip), not kept as a no-op.
