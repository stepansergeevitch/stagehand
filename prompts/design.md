...
- `qa`: at most 10 browser journeys a later automated run executes with claude-in-chrome. Each: id `S1`..`S10`, title, starting URL (real routes from this repo, e.g. `/projects`, `/projects/{id}/edit`), persona, `seed`, and ordered steps of `action` → `assert`; `"shot": true` on the steps whose asserted state must be screenshotted (≥ 1 per scenario). Test the NEW behaviour directly; a regression smoke is a supplement. Zero scenarios ONLY when nothing is user-visible (backend-only, refactor, config) — say why in `qaSkippedReason`.
